# 6.3 Модуль math
"""
Квадратное уравнение 🌶️🌶️

Даны три вещественных числа a, b, c. Напишите программу, которая находит вещественные корни квадратного уравнения: ax ** 2 + bx + c = 0
Если уравнение имеет два корня, то следует вывести их в порядке возрастания.

Формат входных данных
На вход программе подаются три действительных числа a ≠ 0, b,c, каждое на отдельной строке.

Формат выходных данных
Программа должна вывести действительные числа – корни уравнения, каждый на отдельной строке, если они существуют, или текст «Нет корней» в противном случае.

Примечание. Корни квадратного уравнения (если они существуют) ax ** 2 + bx + c = 0 находятся по следующим формулам:

x1 = (-b - квадратный корень из (b ** 2 - 4 * a * c)) / 2 * a
x2 = (-b + квадратный корень из (b ** 2 - 4 * a * c)) / 2 * a

"""

from math import *

a = float(input())
b = float(input())
c = float(input())

D = pow(b,2) - 4 * a * c
if D > 0:
    x1 = ((- b) - sqrt(D)) / (2 * a)
    x2 = ((- b) + sqrt(D)) / (2 * a)
    print(min(x1, x2))
    print(max(x1, x2))
elif D == 0:
    print(-(b / (2 * a)))
else:
    print("Нет корней")